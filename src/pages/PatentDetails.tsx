      ...
      <div className="space-y-8 pb-10">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            {isAssignedDrafter() && (
              <Button 
                onClick={handleCompleteDrafting}
            ...
            {patent.fer_status === 1 && (
              <>
                ...
                <FEREntriesSection 
                  ...
                  employees={employees} 
                  userRole={user?.role}
                  onDraftComplete={handleFERDraftCompletion}
                  onFileComplete={handleFERFilingCompletion}
                  onDraftApprove={handleFERDraftApproval}
                  onFileApprove={handleFERFilingApproval}
                  refreshPatentData={fetchPatentData}
                />
              </>
            )}
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Complete {dialogType === 'complete-draft' ? 'Drafting' : 'Filing'}</DialogTitle>
              <DialogDescription>
                Are you sure you want to mark this task as complete? This will submit it for review.
              </DialogDescription>
            </DialogHeader>
            <div>
              {dialogType === 'complete-draft' && (
                <div className="text-sm">
                  <p className="font-medium">{getDrafterCompletionField()} Task:</p>
                  <p>After completion, this task will be marked for review by an administrator.</p>
                </div>
              )}
              {dialogType === 'complete-file' && (
                <div className="text-sm">
                  <p className="font-medium">{getFilerCompletionField()} Task:</p>
                  <p>After completion, this task will be marked for review by an administrator.</p>
                </div>
              )}
            </div>
        ...
        <Dialog open={showFilingDialog} onOpenChange={setShowFilingDialog}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Please check the form requirements that have been completed for this filing.
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button 
                variant="outline" 
                onClick={() => setShowFilingDialog(false)}
              >
                Cancel
              </Button>
              <Button 
