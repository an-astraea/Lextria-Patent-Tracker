...
export const fetchFilerAssignments = async (filerName: string) => {
  try {
    ...
    // 1. The filer has pending tasks
    // 2. For PS filing: IDF must be received and PS drafting must be complete
    // 3. For CS filing: CS data must be received and CS drafting must be complete
    const filteredPatents = normalizedPatents.filter(patent => 
      ...
      // FER filer task is ready when FER drafting is complete (not changing this workflow for now)
      (patent.fer_filer_assgn === filerName && 
...
export const completeFilerTask = async (patent: Patent, userName: string, formData?: Record<string, boolean>) => {
  try {
    ...
    if (patent.ps_filer_assgn === userName && patent.ps_filing_status === 0) {
      // Check if IDF is received before allowing PS filing completion
      if (!patent.idf_received) {
        toast.error('IDF must be received before PS Filing can be completed');
        return false;
      }
      ...
      // Check if CS data is received before allowing CS filing completion
      if (!patent.cs_data || !patent.cs_data_received) {
        toast.error('CS Data must be sent and received before CS Filing can be completed');
        return false;
      }
